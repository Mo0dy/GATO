...
class MPC_GATO:
    ...
    def init_mpc(
        ...
        stats = {
            'timestamps': [],
            'solve_times': [],        # GPU solve time in seconds
    ...
    def srun_mpc_figure8(
        ...
        state.x_curr = np.concatenate([q, dq])

        # Check if trajectory is complete
        eepos_offset = int(state.total_sim_time / self.dt)
        if eepos_offset >= len(goals)/6 - 6*self.N:
            return False
         
        # Prepare next optimization
        x_curr_batch = np.tile(state.x_curr, (self.batch_size, 1))
        ee_g = goals[6*eepos_offset:6*(eepos_offset+self.N)]
        state.ee_g_batch[:, :] = ee_g
        ...
        state.XU_best = XU_batch_new[best_id, :]
        # Keep diversity in batch; store best separately
        state.XU_batch[:, :] = XU_batch_new

        # Collect essential statistics
        ee_pos = self.solver.ee_pos(q)
        goal_dist = np.linalg.norm(ee_pos[:3] - ee_g[6:9])

        stats['timestamps'].append(state.total_sim_time)
        stats['solve_times'].append(gpu_solve_time/1000.0)  # ms -> s
    ...
    # Hardware helpers
    # -----------------------
    def safety_controller(self, q: np.ndarray, dq: np.ndarray) -> np.ndarray:
        """
        Very simple fallback torque: velocity damping toward zero with gentle centering.
        Returns a torque vector of shape (nu,).
        """
        # Damping
        kv = np.full(self.nu, 10.0)
        # Soft centering around zeros (assumes near-zero home); override as needed
        kp = np.full(self.nu, 20.0)
        q_ref = np.zeros(self.nu)
        tau = -kv * dq[:self.nu] - kp * (q[:self.nu] - q_ref)
        # Optional clamp to reasonable torque limits if solver exposes them
        return np.clip(tau, -100.0, 100.0)

    def predict_state_forward(self, x_curr: np.ndarray, u_last: np.ndarray, lag_s: float) -> np.ndarray:
        """
        Predict state forward by lag_s using RK4 at self.sim_dt steps.
        """
        if lag_s <= 1e-6:
            return x_curr
        steps = max(1, int(lag_s / self.sim_dt))
        h = lag_s / steps
        q = x_curr[:self.nq]
        dq = x_curr[self.nq:]
        for _ in range(steps):
            q, dq = rk4(self.model, self.data, q, dq, u_last, h, self.actual_f_ext)
