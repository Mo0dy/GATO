...
__device__ void integrator_error_inner(T* s_err, T* s_q_next, T* s_qd_next, T* s_q, T* s_qd, T* s_qdd, T dt, T* s_temp)
{
...
__device__ void integrator_gradient_inner(T* s_Ak, T* s_Bk, T* s_dqdd, T dt)
{
        ...
        if (INTEGRATOR_TYPE == 0) {  // euler (v_next = v + a*dt, q_next = q + v*dt)
                ...
                for (uint32_t i = thread_id; i < STATE_SIZE_SQ; i += block_dim) {
                        ...
                        if (r < half_state && r == (c - half_state)) { val += dt; }
                ...
                for (uint32_t i = thread_id; i < STATE_SIZE_SQ; i += block_dim) {
                        ...
                        if (r < half_state) {  // Top rows: dq_next/dx
                                ...
                                if (c >= half_state && r == (c - half_state)) { val += dt; }
                ...
                if (thread_id == 0) { printf("Integrator [%d] not defined. Currently support [0: Euler, 1: Semi-Implicit Euler, 2: Trapezoidal]", INTEGRATOR_TYPE); }
        }
}
