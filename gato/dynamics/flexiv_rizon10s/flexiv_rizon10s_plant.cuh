...
#include <stdio.h>
#include "flexiv_rizon10s_grid.cuh"
...
namespace plant {
        ...
        __device__ constexpr T JOINT_LIMITS_DATA[7][2] = {
            // from flexiv_rizon10s_kinematics.urdf
            {-2.7925 - JOINT_LIMIT_MARGIN<T>(), 2.7925 + JOINT_LIMIT_MARGIN<T>()},  // joint 0
            {-2.6704 - JOINT_LIMIT_MARGIN<T>(), 2.6704 + JOINT_LIMIT_MARGIN<T>()},  // joint 1
            {-2.7925 - JOINT_LIMIT_MARGIN<T>(), 2.7925 + JOINT_LIMIT_MARGIN<T>()},  // joint 2
            {-2.7053 - JOINT_LIMIT_MARGIN<T>(), 2.7053 + JOINT_LIMIT_MARGIN<T>()},  // joint 3
            {-2.9671 - JOINT_LIMIT_MARGIN<T>(), 2.9671 + JOINT_LIMIT_MARGIN<T>()},  // joint 4
            {-1.3963 - JOINT_LIMIT_MARGIN<T>(), 4.5379 + JOINT_LIMIT_MARGIN<T>()},  // joint 5
            {-2.9671 - JOINT_LIMIT_MARGIN<T>(), 2.9671 + JOINT_LIMIT_MARGIN<T>()}   // joint 6
        };

        template<class T>
        __device__ constexpr T VEL_LIMITS_DATA[7][2] = {
            // from flexiv_rizon10s_kinematics.urdf
            {-1.7453 - JOINT_LIMIT_MARGIN<T>(), 1.7453 + JOINT_LIMIT_MARGIN<T>()},  // joint 0
            {-1.7453 - JOINT_LIMIT_MARGIN<T>(), 1.7453 + JOINT_LIMIT_MARGIN<T>()},  // joint 1
            {-2.0944 - JOINT_LIMIT_MARGIN<T>(), 2.0944 + JOINT_LIMIT_MARGIN<T>()},  // joint 2
            {-2.0944 - JOINT_LIMIT_MARGIN<T>(), 2.0944 + JOINT_LIMIT_MARGIN<T>()},  // joint 3
            {-3.8397 - JOINT_LIMIT_MARGIN<T>(), 3.8397 + JOINT_LIMIT_MARGIN<T>()},  // joint 4
            {-3.8397 - JOINT_LIMIT_MARGIN<T>(), 3.8397 + JOINT_LIMIT_MARGIN<T>()},   // joint 5
            {-3.8397 - JOINT_LIMIT_MARGIN<T>(), 3.8397 + JOINT_LIMIT_MARGIN<T>()}   // joint 6
        };

        template<class T>
        __device__ constexpr T CTRL_LIMITS_DATA[7][2] = {
            // from flexiv_rizon10s_kinematics.urdf
            {-261.0 - JOINT_LIMIT_MARGIN<T>(), 261.0 + JOINT_LIMIT_MARGIN<T>()},  // joint 0
            {-261.0 - JOINT_LIMIT_MARGIN<T>(), 261.0 + JOINT_LIMIT_MARGIN<T>()},  // joint 1
            {-123.0 - JOINT_LIMIT_MARGIN<T>(), 123.0 + JOINT_LIMIT_MARGIN<T>()},  // joint 2
            {-123.0 - JOINT_LIMIT_MARGIN<T>(), 123.0 + JOINT_LIMIT_MARGIN<T>()},  // joint 3
            {-57.0 - JOINT_LIMIT_MARGIN<T>(), 57.0 + JOINT_LIMIT_MARGIN<T>()},  // joint 4
            {-57.0 - JOINT_LIMIT_MARGIN<T>(), 57.0 + JOINT_LIMIT_MARGIN<T>()},   // joint 5
            {-57.0 - JOINT_LIMIT_MARGIN<T>(), 57.0 + JOINT_LIMIT_MARGIN<T>()}   // joint 6
        };
